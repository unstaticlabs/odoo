...
class FakeDatabase:
    ...
    def tables(self):
        return [
            ...
            {"table_name": "ai_agent", "record_count": 1},
        ]
...
class SourceTruthAuditCase(unittest.TestCase):
    ...
    def test_incomplete_scope_blocks_inventory(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(
                report["blocked_scopes"]["ai_configuration"]["fields"][0]["name"],
                "system_prompt",
            )

    def test_approved_source_configuration_drops_are_explicit_and_non_blocking(self):
        contract = load_contract(ROOT / "migration/source_truth/coverage.json")
        for scope in ("ai_configuration", "knowledge", "sales_marketing"):
        ...
        self.assertEqual(contract["scopes"]["studio"]["status"], "implemented")
        self.assertEqual(contract["scopes"]["studio"]["disposition"], "recomputed")
