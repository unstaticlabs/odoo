...
description: Synchronize the USL Odoo distribution with a newer upstream Odoo revision while preserving ancestry and isolating compatibility work. Use for upstream catch-up, upstream merge preparation, conflict qualification, or a sync handoff to the Lead Developer.
---
...
7. Record the upstream range, conflicts, USL compatibility commits, upgrade/recovery plan, failures, and unverified areas in the normal v1 feature handoff.
8. Push through `scripts/agent/github push`, open/update the PR through the helper, and stop for Lead Developer integration.
