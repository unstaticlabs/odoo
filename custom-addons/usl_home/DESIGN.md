...
typography:
  title:
    fontFamily: '"SF Pro Display", -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, "Helvetica Neue", Ubuntu, "Noto Sans", Arial, sans-serif'
    fontSize: "1.75rem"
    fontWeight: 650
    lineHeight: 1.2
  ...
  headline:
    ...
    lineHeight: 1.2
  body:
    fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, "Helvetica Neue", Ubuntu, "Noto Sans", Arial, sans-serif'
    fontSize: "0.875rem"
    fontWeight: 400
    lineHeight: 1.5
  label:
...
The surface inherits Odoo and Bootstrap typography, controls, navbar behavior, semantic variables, and action language. Its own visual contribution is deliberately restrained: a light canvas, content-sized white cards, fine borders, compact operational copy, and a small amount of semantic color reserved for state. Customization is available but visually secondary to action and continuation.
...
- Exact destinations and exceptions are presented as direct action rows.
...
Desktop layout uses a 12-column grid with one-rem gaps. Activities and Favorite Views span seven columns; My Tasks and AI Pipelines span five; Accounting & Compliance Alerts spans the full width. This creates two content-sized columns without forcing matched heights. At widths below 992 pixels every widget becomes full-width, while the accounting alert tiles reduce from five columns to two. Below 576 pixels the page padding tightens, header actions stack, customization becomes one column, task signals become two-by-two, and accounting alerts become a single column.
...
The form language is gently rounded but still native and workmanlike. Main widgets use the largest local radius, customization and accounting alert tiles use the middle radius, and loading skeletons use the smallest radius. One-pixel borders remain visible, and widget overflow is clipped to preserve rounded silhouettes. Row actions stay rectangular and borderless within their parent cards so the card, not each row, is the primary container.
...
- **Content:** A truncated title and muted metadata lead; timing, status, or an arrow remains fixed at the trailing edge.
...
- **Content:** A tabular numeric value sits above a small muted label.
...
- **Content:** A bold tabular count, alert label, and trailing arrow form a single direct navigation target.
