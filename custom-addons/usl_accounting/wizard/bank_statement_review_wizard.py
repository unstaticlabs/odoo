...
class AccountBankStatementConfirm(models.TransientModel):
    ...
    statement_id = fields.Many2one("account.bank.statement", required=True, readonly=True)
    ...
    movement_total = fields.Monetary(related="statement_id.movement_total", readonly=True)
    ...
    def action_confirm(self):
        ...
        if self.statement_id.certification_state == "certified":
            raise UserError(_("Reopen the certified statement before changing its balances."))
        ...
        self.statement_id.message_post(body=_("Bank-reported opening and closing balances confirmed."))
        return {
            ...
            "params": {
                "message": (
                    ...
                    if self.statement_id.currency_id.compare_amounts(self.resulting_difference, 0) == 0
                    else _("Balances saved. Resolve the displayed difference before certification.")
                ),
                "type": "success" if self.statement_id.currency_id.compare_amounts(self.resulting_difference, 0) == 0 else "warning",
                "sticky": False,
...
class AccountBankStatementReopen(models.TransientModel):
    ...
    def action_reopen(self):
        ...
        if not self.env.user.has_group("account.group_account_manager"):
            raise AccessError(_("Only an Accounting Manager can reopen a certified statement."))
        ...
        statement.message_post(
            body=_("Certified bank statement reopened. Reason: %(reason)s", reason=self.reason.strip())
        )
        return {
            "type": "ir.actions.client",
            "tag": "display_notification",
