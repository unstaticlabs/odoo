...
class AccountBankIngestionFile(models.Model):
    ...
    paperless_operation_id = fields.Many2one(
        "usl.document.operation", copy=False, readonly=True, ondelete="set null"
    )
    paperless_document_id = fields.Many2one(
        "usl.document", copy=False, readonly=True, ondelete="set null"
    )
    ...
    def _archive_or_reconcile_bank_evidence(self):
        ...
        existing_link = self.env["usl.document.link"].sudo().search(
            [
        ...
        try:
            if existing_link:
                result = existing_link.document_id.with_user(
                    ...
                    version_label=_("Bank statement %(period)s", period=self.period_end),
                )
            else:
        ...
        self.sudo().write(
            {
                "paperless_archive_state": (
                    "archived" if result.get("state") == "duplicate" and document else "processing"
                ),
