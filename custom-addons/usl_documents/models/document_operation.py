...
class UslDocumentOperation(models.Model):
    ...
    def poll(self):
        for operation in self.filtered(
            ...
            if status in ("success", "successful"):
                operation._poll_succeeded_task(task)
            elif status in ("failure", "failed"):
                operation._poll_failed_task(task)
    ...
    def _poll_task_paperless_id(self, task):
        """Return the Paperless document a successful task produced, or None."""
        self.ensure_one()
        ...
        if not paperless_id and self.target_document_id:
            paperless_id = self.target_document_id.paperless_id
        if not paperless_id:
            return None
    ...
    def _poll_succeeded_task(self, task):
        """Mirror the archived document of a successful Paperless task."""
        self.ensure_one()
        operation = self
        paperless_id = operation._poll_task_paperless_id(task)
        if not paperless_id:
            return
        ...
        try:
            ...
            return
        document = operation._poll_cache_document(client, payload, paperless_id)
        if document is None:
            return
        operation._poll_bind_versions(document, payload)
        if missing:
            operation._poll_flag_missing_source(document, archive_context, missing)
            return
        if archive_context:
    ...
    def _poll_cache_document(self, client, payload, paperless_id):
        """Write the Paperless payload into the cached document of this operation.

        Return None after marking the operation as a duplicate when Paperless
        matched identical content with a different classification fingerprint.
        """
        self.ensure_one()
        operation = self
        metadata_catalog = None
        if self.env["usl.document"]._needs_metadata_catalog([payload]):
        ...
        return document_cache.create(values)

    def _poll_bind_versions(self, document, payload):
        """Align original timestamps and the current version with this operation."""
        self.ensure_one()
        operation = self
    ...
    def _poll_flag_missing_source(self, document, archive_context, missing):
        """Archive the document for review when its source record disappeared."""
        self.ensure_one()
        operation = self
    ...
    def _poll_failed_task(self, task):
        """Reuse an identical archive or record the failure of a Paperless task."""
        self.ensure_one()
        operation = self
