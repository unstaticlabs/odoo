...
DEV_QA_DB = "odoo_saas_19_2_candidate_01"
    ...
    access_script.write_text(
        "\n".join(
            [
                ...
                "        privacy_probe_exception_type = type(exc).__name__",
                "review_decision_access = {operation: ReviewDecision.check_access_rights(operation, raise_exception=False) for operation in ('read', 'write', 'create', 'unlink')}",
                "external_value_access = {operation: ExternalValue.check_access_rights(operation, raise_exception=False) for operation in ('read', 'write', 'create', 'unlink')}",
