...
def _write_adopt_generation(
    ...
    _write_remote(
        ...
        _generation_overlay(
            ...
            deployment_identity=_deployment_identity(target, release, generation),
        ),
        "0644",
    )
    return release_path
...
def _release_images(release: dict, mcp_authority: dict | None = None) -> list[str]:
    ...
    images.update(
        release["components"][name]["digest_reference"]
...
def _validate_runtime_release_images(target, runner, runtime: dict, release: dict) -> dict[str, str]:
    ...
    for component, service_key in RELEASE_RUNTIME_SERVICES.items():
        if component in {"receipt-fetcher", "receipt-egress"} and component not in release["components"]:
            continue
        service = target.value["services"][service_key]
    ...
    try:
        ...
        _write_remote(
            ...
            _generation_overlay(
                ...
                deployment_identity=_deployment_identity(target, release),
            ),
            "0600",
        )
...
def health_command(arguments: argparse.Namespace) -> int:
    ...
    release, _release_sha256, _release_raw = _release(target, runner, None)
    receipt_enabled = "receipt-fetcher" in release["components"]
    expected = set(target.value["services"].values())
    if not receipt_enabled:
        expected -= {
            target.value["services"].get("receipt_fetcher"),
            target.value["services"].get("receipt_egress"),
        }
    ...
    if config_result.returncode:
        ...
        try:
            ...
            if receipt_enabled and target.value["environment"] == "production" and odoo_config.get("workers", 0) < 4:
                failures.append("odoo:receipt-worker-capacity")
            modules = {
                value.strip()
                for value in odoo_config.get("server_wide_modules", "").split(",")
            }
            if receipt_enabled and "queue_job" not in modules:
                failures.append("odoo:queue-job-not-loaded")
            channels = odoo_config.get("queue_channels", "").replace(" ", "")
            if receipt_enabled and "root.receipt_fetch:2" not in channels.split(","):
...
"""
    ...
    receipt_mtls = None
    if receipt_mtls_result is None:
        pass
    elif receipt_mtls_result.returncode:
...
"""
    fetcher_service = target.value["services"].get("receipt_fetcher")
    ...
    if containment_result is None:
        pass
    elif containment_result.returncode:
...
def _deployment_identity(target, release: dict, generation: str | None = None) -> dict[str, str]:
    """Describe the selected Odoo release and the verified executing GitOps bundle."""
    gitops_commit = "Unknown"
    if os.environ.get("USL_RELEASE_GITOPS_ROOT") or os.environ.get("USL_RELEASE_GITOPS_COMMIT"):
        _root, gitops_commit = _gitops_root()
    return {
        "USL_DEPLOYMENT_ENV": target.value["environment"],
        "USL_RELEASE_COMMIT": release.get("source", {}).get("commit", "Unknown"),
        "USL_GITOPS_COMMIT": gitops_commit,
        "USL_DEPLOYMENT_GENERATION": generation or "Unknown",
...
def _generation_overlay(
    ...
    if release is not None:
        ...
        images.update({
            name: release["components"][name]["digest_reference"]
            for name in ("receipt-fetcher", "receipt-egress")
            if name in release["components"]
        })
        value["services"] = {
        ...
        if deployment_identity:
            for service in (odoo_service, "odoo-upgrade", "init-db"):
                if service in value["services"]:
                    value["services"][service].setdefault("environment", {}).update(deployment_identity)
...
def _previous_generation_identity(target, runner, current: dict) -> tuple[dict, str | None]:
    ...
    if (
        ...
        or set(volumes) not in (
            set(target.value["volumes"]),
            set(target.value["volumes"]) - {"receipt_control"},
        )
    ...
    if (
        "receipt-fetcher" in previous_release.get("components", {})
        and "receipt_control" not in volumes
    ):
        raise RuntimeError("rollback receipt sandbox volume is missing")
