...
def validate_target(payload: object, path: Path = Path("<memory>")) -> Target:
    ...
    receipt_services = {"receipt_fetcher", "receipt_egress"}
    if isinstance(services, dict) and receipt_services & set(services):
        required_services |= receipt_services
    ...
    if receipt_services <= set(services):
        required_volumes.add("receipt_control")
    elif isinstance(volumes, dict) and "receipt_control" in volumes:
        raise RuntimeError("receipt_control requires both receipt services")
...
def read_active_state(target: Target, runner: Runner) -> dict[str, Any] | None:
    ...
    if set(state["volumes"]) != set(target.value["volumes"]):
        historical_roles = set(target.value["volumes"]) - {"receipt_control"}
        if set(state["volumes"]) != historical_roles:
            raise RuntimeError("active generation volume perimeter differs")
        release = read_active_release(target, runner, state)
        if "receipt-fetcher" in release["components"]:
            raise RuntimeError("receipt release requires its control volume")
...
def effective_volumes(target: Target, runner: Runner) -> tuple[dict[str, dict[str, str]], str | None]:
    ...
    if state is None:
        definitions = target.value["volumes"]
        if "receipt_control" in definitions:
            try:
                release = validate_release(json.loads(runner.run([
                    "cat", target.value["release_manifest"],
                ]).stdout))
            except (json.JSONDecodeError, ReleaseManifestError) as error:
                raise RuntimeError("adopted release manifest is invalid") from error
            if "receipt-fetcher" not in release["components"]:
                definitions = {role: value for role, value in definitions.items() if role != "receipt_control"}
