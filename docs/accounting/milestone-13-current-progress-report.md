...
- Track B now has a separate disposable `odoo_rebuild_accounting_track_b` database and clean native expense, business-document, expense-settlement, direct document/bank-settlement and non-bank General Reconciliation replay for `2025-10-01` through `2026-06-30`. All `325` source expenses pass native submission, approval/refusal, receipt and company-payment workflows with `0` blocked/mismatched records; the replay creates `97` company payments and `79` grouped receipts for `95` employee-paid expenses, while preserving `125` approved, `3` draft and `5` refused records. All `284` source documents (`36` customer invoices, `161` vendor bills, `3` supplier refunds and `84` purchase receipts) are then created or reused from commercial fields and posted through normal Odoo APIs; `284/284` match source header amounts, due dates and per-account debit/credit/balance/amount-currency effects, with `0` blocked cases and `0` mismatches across `170` EUR and `114` USD documents. Expense settlement creates `106` native bank transactions and replays `181` source allocations through OCA against `176` expense settlement lines; all `97` company payments and `95` employee-paid expenses finish paid. Document settlement then covers all `233` directly linked bank transactions and `339` source document/bank edges: it reuses `8` bank lines and `83` partials from expense settlement, creates `225` bank lines and `256` additional partials, and validates all company/transaction-currency partial amounts, endpoints and `241` due-line residuals. General Reconciliation covers all `111` non-bank document partials and `114` document endpoints by posting `18` manual entries with `66` lines, creating `71` input partials and tracing `40` native exchange partials. Final document payment-state counts exactly match source; a rerun creates nothing and reuses all stage traces. `170` bank lines are complete within the document perimeter; `63` retain bounded outside-perimeter aggregate detail for later bank-wide coverage. Six mixed reimbursement transfers previously retained `5,942.24` on account `455100`; the document stage consumes the one additional current receipt edge through General Reconciliation. Two remaining legacy single-line documents use Odoo's supported manual-tax metadata and are explicitly classified. No finalized source journal line is passed into document or expense creation.
...
- full-period bank/general-reconciliation coverage outside the now-proved `114` non-bank document endpoints, including outside-perimeter aggregate allocations, write-offs and undo
- the remaining `1,510` current-period bank transactions outside the union of the `106` expense and `233` document-linked lines, plus their payment/reconciliation/exchange-difference effects
