...
| Currencies and Rates | All restored rates and automated ECB reference rates share native `res.currency.rate`; manager execution fills every missing published day after the protected coverage boundary, and daily automation recovers recent gaps. | Implemented | Exact broad-snapshot comparison, multi-day backfill, source/manual preservation, idempotence and provider evidence. |
