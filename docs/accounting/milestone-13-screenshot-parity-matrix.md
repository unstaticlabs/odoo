...
| Review supplier evidence | Native bill, original PDF thumbnail and viewer route. | Imported native attachment binary is readable through the allowed accounting record. | Manager browser and reviewer regression test passed in Track B; the records are integrated into the disposable hybrid candidate, whose browser journey and promotion are pending. |
| Review expense evidence | Native expense, approval/accounting state, original receipt filename and thumbnail. | Expense navigation is available within the allowed company. | Manager browser and Track B reviewer navigation evidence passed; the records are integrated into the disposable hybrid candidate, whose browser journey and promotion are pending. |
