...
| 6 | Bank Matching | Import, categorize and reconcile bank transactions while preserving operator inputs and residual review. | Each bank journal provides contextual CAMT, QIF and mapped CSV/XLSX import; maintained OCA Bank Matching handles operational matching. Imported historical relationships remain read-only evidence and unmatched items remain visible on Accounting Home. | Equivalent replacement | Fresh-database tests create a normal statement, transaction and retained source attachment from QIF. The manager browser journey proves a contextual import, full-width Transactions, exact-reference payment matching, visible counterpart lines and Undo. Reviewer mutation controls remain denied. Live bank synchronization is deferred beyond Milestone 13. |
