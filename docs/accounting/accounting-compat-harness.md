...
The validation status is deliberately `partial`, classified as `HYBRID_REPLACEMENT_TARGET_EXPLAINED_NATIVE_DIFFERENCES`. Professional acceptance of the EUR `2.64` difference, replacement report/role/browser journeys and an explicit promotion decision remain required. The disposable candidate does not replace `odoo_rebuild_accounting_test` or authorize production migration.
