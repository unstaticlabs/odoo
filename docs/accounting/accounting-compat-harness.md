...
These read-only views are the audit/evidence layer under `Review > Advanced Audit`, which is restricted to system administrators and development diagnostics. Accounting managers and read-only accountants reach reports through the canonical Accounting report pages and never see reconstruction objects in their normal menus. The reports stage records both audit-view controls and live workbench controls in `reports-status.json`, including sampled source actions proving that ledger-backed report rows open contributing `account.move.line` records and analytic report rows open `account.analytic.line`.
