...
- Apply environment-specific target configuration only after source parity and
  migration finalization. `migration/manage` is the sole reconstruction and
  cutover interface. Only a fresh-source transition reconstruction may claim
  source-wide migration evidence; it requires the strict source and attachment
  gates before target initialization.
