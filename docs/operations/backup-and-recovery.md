...
Receipt-fetcher releases add a paired `receipt-fetcher` / `receipt-egress`
component contract and a transient `receipt_control` volume. New releases must
include the complete pair. Baseline verification and recovery still accept
historical v2/v3 manifests without either component; they must not inspect or
require a receipt volume that the recorded release never created. A missing
component or control volume in a receipt-enabled release remains an error.
