...
7. Check the proposed feedback. Choose **Add details** to revise it or **Send
   to product team** when it is accurate.

The **Board** shows product feedback to internal employees. Everyone can read
and discuss feedback and its files. Only feedback maintainers can change task
fields or workflow stages. A shared feedback task never grants access to the
...
one. The assistant works without the optional Projects MCP connection and
falls back to the saved conversation if its background mode is unavailable.
Projects MCP only improves awareness of existing feedback. Feedback
