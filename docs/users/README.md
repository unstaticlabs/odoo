...
Expense reconstruction status: a separate, disposable Track B proof reconstructs all `325` source expenses from `2025-10-01` through `2026-06-30` as source-traced native `hr.expense` records and validates their approval/refusal states and accounting effects without mismatches. This includes `97` company payments and `79` grouped receipts for `95` employee-paid expenses. A bounded settlement stage also replays `106` expense-related bank transactions and `181` source allocations through the OCA workbench, leaving all `97` company payments and `95` reimbursed expenses paid. The next Track B stage proves all `233` bank transactions directly linked to the current commercial documents and their `339` source reconciliation edges, including transaction-currency amounts and open due-line residuals. The normal imported target still preserves the exact historical ledger rather than mixing in these recomputed records; making the proven current-period workflow operational there remains a separate integration increment. Until then, review historical expense and document settlement in the imported ledger/evidence and use the native workspaces only for new daily activity.
