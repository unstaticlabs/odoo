...
| Shared conversational product feedback with a private local Odoo-page preview, opt-in sanitized page context, Gemini-assisted clarification and human-confirmed Triage | `usl_feedback` over native Project, mail and Settings models | Persistent **Feedback** action beside **New Message** in the top-right messages drawer; native-style floating ChatHub conversation; shared **Board**; **Product Feedback** for maintainers | [Product contract](product-feedback.md), [user how-to](../users/how-to/send-product-feedback.md), [access-risk inventory](../agents/distribution-access-risk-inventory.md) | **Feature candidate**; human approval is required after Feature/Worktree-QA review before Lead handoff |
