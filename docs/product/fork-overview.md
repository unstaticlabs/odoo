...
| Shared conversational product feedback with a private local Odoo-page preview, default-selected sanitized page details, live Gemini-assisted clarification, reporter withdrawal and human-confirmed Triage | `usl_feedback` over native Project, mail and Settings models | Persistent **Feedback** action beside **New Message**; floating native Thread/Composer with live follow-up replies, the active **Feedback #…** identity/stage/status, **My feedback** tracking and **New**; shared **Board**; **Product Feedback** for maintainers | [Product contract](product-feedback.md), [user how-to](../users/how-to/send-product-feedback.md), [access-risk inventory](../agents/distribution-access-risk-inventory.md) | **Feature candidate**; human approval is required after Feature/Worktree-QA review before Lead handoff |
...
| `usl_feedback` | Shared conversational product feedback, screen evidence, stateful Gemini shaping and confirmed native Project workflow | Delivered product feature on the human-gated integration candidate |
