...
| Private authenticated product feedback with opt-in sanitized page context, native task workflow and reporter/maintainer isolation | `usl_feedback` over native Project and mail models | **Send feedback** and **My feedback** in the user menu; **Product Feedback** for maintainers | [Product contract](product-feedback.md), [user how-to](../users/how-to/send-product-feedback.md), [access-risk inventory](../agents/distribution-access-risk-inventory.md) | **Feature candidate**; implementation and isolated QA evidence are owned by the feature PR until Lead integration |
...
| `usl_feedback` | Authenticated private product feedback, sanitized page context and native Project task routing | Delivered product feature on the integration candidate |
