...
def artifact_manifest(candidate_dir: Path) -> dict:
    ...
    for relative in REQUIRED_ARTIFACTS:
        ...
        digest, file_count, size = qa_seed.tree_digest(path)
        result[relative] = {
...
def build_payload(
    ...
    if source_dir:
        source_dump_sha256 = qa_seed.sha256_file(source_dir / "dump.sql")
        source_filestore_sha256 = qa_seed.tree_digest(source_dir / "filestore")[0]
    else:
    ...
    migration_sha256 = qa_seed.migration_digest(root)
    ...
    if filestore.get("archive_sha256") != qa_seed.sha256_file(
        candidate_dir / "odoo-filestore.tgz",
    ):
    ...
    payload = {
        ...
        "qualification_sha256": qa_seed.sha256_file(
            candidate_dir / "qualification.json",
        ),
        "release_identity_sha256": qa_seed.sha256_file(
