...
SAFE_PROJECT = re.compile(r"^usl-odoo-production-[a-z0-9][a-z0-9-]*$")
VOLUME_KEYS = (
...
def validate_environment(values: dict[str, str], candidate: dict) -> None:
    ...
    for key in VOLUME_KEYS:
        name = values[key]
        if not name.startswith(project + "-") or not re.fullmatch(
            r"[a-z0-9][a-z0-9_.-]+",
            name,
        ):
            raise CutoverError(f"{key} is not candidate-owned by {project}")
