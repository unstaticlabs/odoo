...
ODOO_CHECKPOINTS = (
    "codex/checkpoint/documents-policy-engine",
    "codex/checkpoint/paperless-local-hybrid-search",
    "codex/checkpoint/documents-search-ux",
    "codex/checkpoint/personal-gemini-opt-in",
    "codex/checkpoint/portable-release-cohort",
    "codex/checkpoint/documents-production-candidate",
)
MCP_CHECKPOINTS = (
    "codex/checkpoint/documents-mcp-foundation",
    "codex/checkpoint/documents-mcp-acceptance",
    "codex/checkpoint/documents-mcp-production-candidate",
)
PRODUCT_MODULES = (
...
def load_qa_seed_module():
    spec = importlib.util.spec_from_file_location(
        "qa_seed_release_identity",
        ROOT / "scripts/qa_seed.py",
    )
    if spec is None or spec.loader is None:
        raise IdentityError("qa_seed identity module cannot be loaded")
...
def checkpoint_map(repository: Path, names: tuple[str, ...]) -> tuple[dict, list[str]]:
    result = {}
    missing = []
    for name in names:
        completed = subprocess.run(
...
def build(args: argparse.Namespace) -> dict:
    ...
    odoo_checkpoints, missing_odoo = checkpoint_map(ROOT, ODOO_CHECKPOINTS)
    mcp_checkpoints, missing_mcp = checkpoint_map(mcp_repository, MCP_CHECKPOINTS)
    ...
    blockers.extend(f"missing checkpoint {name}" for name in missing_odoo + missing_mcp)
    if odoo_status:
    ...
    source_filestore_sha, source_files, source_size = qa_seed.tree_digest(filestore)
    identity = {
        ...
        "source": {
            ...
            "migration_code_sha256": qa_seed.migration_digest(ROOT),
        },
        "git": {
            "odoo_branch": run(ROOT, "git", "branch", "--show-current"),
            "odoo_commit": odoo_commit,
            "odoo_checkpoints": odoo_checkpoints,
            "odoo_mcp_branch": run(
                mcp_repository,
                "git",
                "branch",
            ...
            "odoo_mcp_checkpoints": mcp_checkpoints,
        },
